...
module.exports = function() {
  ...
  for (let i = 0; i < config.settings.NUM_ASTEROIDS; i++) {
    let asteroidX = Math.floor(Math.random() * (config.settings.PLAYAREA_WIDTH - 800)) + 800;
    let asteroidY = Math.floor(Math.random() * (config.settings.PLAYAREA_HEIGHT - 700)) + 700;
